...
function FAQItem({ faq, index, visible }) {
  ...
  return (
    <div
      className={`
        border border-[#2E2C2A] rounded-2xl overflow-hidden
    ...
    >
      <button
        ...
        aria-expanded={open}
        style={{ background: open ? 'rgba(232,160,32,0.04)' : '#1A1917' }}
      >
        <span className="font-display font-semibold text-[15px] md:text-[16px] text-[#F2EDE8] leading-[1.3]">
          {faq.question}
        </span>
        <span
          className="shrink-0 w-6 h-6 rounded-full flex items-center justify-center transition-transform duration-300"
          style={{
            background: open ? '#E8A020' : 'rgba(232,160,32,0.1)',
            transform: open ? 'rotate(45deg)' : 'rotate(0deg)',
          }}
          aria-hidden="true"
        >
          <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
            <path d="M6 2v8M2 6h8" stroke={open ? '#0F0E0D' : '#E8A020'} strokeWidth="1.8" strokeLinecap="round"/>
          </svg>
        </span>
      </button>
      ...
      >
        <p className="px-6 pb-5 font-body text-[14px] md:text-[15px] text-[#9C9490] leading-[1.7]">
          {faq.answer}
        </p>
      </div>
    </div>
  ...
  return (
    ...
    >
      ...
      <div className="max-w-[1200px] mx-auto px-5 md:px-8">
        ...
        >
          <p className="font-body text-[15px] text-[#9C9490] mb-4">
            ¿Tienes alguna otra pregunta?{' '}
            <a
              href="mailto:hola@pixelagents.com"
              className="text-[#E8A020] hover:underline transition-colors"
            >

...
function MetricCounter({ value, suffix, prefix, label, visible, delay }) {
  ...
  return (
    ...
    >
      <div className="font-display font-extrabold text-[#F2EDE8] mb-1.5" style={{ fontSize: 'clamp(32px, 4vw, 48px)', lineHeight: '1' }}>
        {prefix && <span className="text-[#E8A020]">{prefix}</span>}
        {formatValue(count)}
        {suffix && <span className="text-[#E8A020]">{suffix}</span>}
      </div>
      <p className="font-body text-[13px] text-[#9C9490] leading-[1.4] max-w-[130px]">
        {label}
      </p>
    </div>
  );
...
const avatarColors = ['#E8A020', '#C4821A', '#FFC84D'];

function StarRating() {
  return (
    <div className="flex gap-0.5 mb-4" aria-label="5 de 5 estrellas">
      {[...Array(5)].map((_, i) => (
        <svg key={i} width="14" height="14" viewBox="0 0 24 24" fill="#E8A020" aria-hidden="true">
...
function TestimonialCard({ testimonial, index, visible }) {
  return (
    <article
      className={`
        flex flex-col p-6 rounded-2xl border border-[#2E2C2A]
    ...
    >
      ...
      {/* Quote */}
      <blockquote className="font-body text-[14px] md:text-[15px] text-[#9C9490] leading-[1.7] flex-1 mb-5 italic">
        "{testimonial.quote}"
      </blockquote>

      {/* Author */}
      <div className="flex items-center gap-3 pt-5 border-t border-[#2E2C2A]">
        <div
          className="w-9 h-9 rounded-full flex items-center justify-center font-display font-bold text-[12px] text-[#0F0E0D] shrink-0"
        ...
        <div className="min-w-0">
          <p className="font-display font-semibold text-[14px] text-[#F2EDE8] leading-none mb-0.5 truncate">
            {testimonial.name}
          </p>
          <p className="font-body text-[12px] text-[#9C9490] truncate">
      ...
      >
        <p className="font-display font-semibold text-[12px] text-[#E8A020] flex items-center gap-1.5">
  ...
  return (
    ...
    >
      ...
      <div className="max-w-[1200px] mx-auto px-5 md:px-8">
        ...
        >
          <h2
            className="font-display font-bold text-[#F2EDE8]"
        ...
        >
          <div
            className="rounded-2xl border border-[#2E2C2A] bg-[#0F0E0D] grid grid-cols-2 md:grid-cols-4 divide-x divide-y md:divide-y-0 divide-[#2E2C2A] overflow-hidden"
            role="list"
